...
def create_data(corpus_path):#构建数据，先后使用doc2bow和tfidf model对文本进行向量表示
    ...
    for line in open(corpus_path):
        ...
        if len(line) == 2:
            ...
            break
    #对文本进行处理，得到文本集合中的词表
    dictionary = corpora.Dictionary(sentences) #(4732,)
    #利用词表，对文本进行cbow表示
    corpus = [dictionary.doc2bow(text) for text in sentences] #(1266,)
    #利用cbow，对文本进行tfidf表示
    tfidf = TfidfModel(corpus) #TfidfModel(num_docs=1266, num_nnz=25302)
    corpus_tfidf = tfidf[corpus] #(1266,)
    return sentence_dict, dictionary, corpus, corpus_tfidf

def lda_model(sentence_dict, dictionary, corpus, corpus_tfidf, cluster_keyword_lda):#使用lda模型获取主题分布
    ...
    corpus_lda = lda[corpus_tfidf] #(1266, )
